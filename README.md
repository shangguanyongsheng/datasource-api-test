...
**关键修复**
- ✅ 彻底解决内存崩溃问题：将四层笛卡尔积的 `itertools.product` 改为嵌套循环
- ✅ 添加安全阈值检查，组合数过大时自动限制各层组合数
- ✅ 添加 `_limit_generator` 函数，精确控制各层生成器产出数量

**优化说明**
- 原问题：`itertools.product(filter, dimension, index, order)` 会一次性将所有生成器的值收集到内存
- 解决方案：改用四层嵌套 `for` 循环，配合 `break` 提前退出，实现真正的惰性生成
- 效果：即使组合数达数亿级别，内存占用仍保持稳定
